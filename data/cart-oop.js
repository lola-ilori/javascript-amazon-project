//turning the cart functions in cart.js to an object of oop.
...
function Cart(localStorageName) {
  const cart = {
    ...
    loadFromStorage() { //shortcut for 'function loadFromStorage(){}'
     this.cartObjects = JSON.parse(localStorage.getItem(localStorageName)); //get the cart array from local storage.
   ...
   saveToStorage() {
     localStorage.setItem(localStorageName, JSON.stringify(this.cartObjects)); //save the cart array to anyname(localStorageName) which will be added when we call the function below. JSON.stringify converts the cart array to a string so it can be saved.
   },
 ...
 return cart; //return the cart object so that it can be used in other files
 
}

//creating a cart that can be used for business. OOp allows to build several carts 
const cart = Cart('cart-oop'); //store the cart object in a variable & pass a random localStorageName
const cartBusiness = Cart('cart-business'); //store the cart object in a variable & pass a random localStorageName


cart.loadFromStorage(); //load the cart array from local storage
cartBusiness.loadFromStorage(); //load the cart array from local storage
